horizon.optimizers = {
  ...
  init_rule_list: function() {
    horizon.optimizers.rules_selected = [];
    horizon.optimizers.rules_available = [];
    $(this.get_rule_element("")).each(function(){
      ...
      if($input.is(':checked')) {
        horizon.optimizers.rules_selected.push(rule_property);
      } else {
        horizon.optimizers.rules_available.push(rule_property);
      }
    });
  },
  ...
  init_router_list: function() {
    horizon.optimizers.routers_selected = [];
    horizon.optimizers.routers_available = [];
    $(this.get_router_element("")).each(function(){
      ...
      if($input.is(':checked')) {
        horizon.optimizers.routers_selected.push(router_property);
      } else {
        horizon.optimizers.routers_available.push(router_property);
      }
    });
  },
  ...
  workflow_init: function() {
    ...
    horizon.optimizers.generate_rulelist_html();
    horizon.optimizers.generate_routerlist_html();
  }
};

horizon.addInitFunction(horizon.optimizers.init = function () {
  $(document).on('submit', '#tail_length', function (evt) {
    horizon.optimizers.user_decided_length = true;
    horizon.optimizers.getConsoleLog(true);
    evt.preventDefault();
  });
});

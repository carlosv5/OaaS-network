...
class AddRuleAction(workflows.Action):
    ...
    class Meta(object):
        ...
        help_text = _("Create a optimizer rule.\n\n"
...
class AddRule(workflows.Workflow):
    ...
    success_url = "horizon:project:optimizers:index"
    # oaas is designed to support a wide range of vendor
    # optimizers. Considering the multitude of vendor optimizer
    # features in place today, optimizer_rule definition can
    # involve more complex configuration over time. Hence,
    # a workflow instead of a single form is used for
    # optimizer_rule add to be ready for future extension.
    ...
    def handle(self, request, context):
        try:
            api.oaas.rule_create(request, **context)
            return True
    ...
    def populate_rule_choices(self, request, context):
        try:
            ...
            rule_list = [(rule.id, rule.name_or_id) for rule in rules
                         if not rule.optimizer_policy_id]
        except Exception as e:
            rule_list = []
...
class SelectRulesStep(workflows.Step):
    ...
    template_name = "project/optimizers/_update_rules.html"
    contributes = ("optimizer_rules",)
...
class SelectRoutersAction(workflows.Action):
    router = forms.MultipleChoiceField(
        ...
        required=False,
        widget=forms.CheckboxSelectMultiple(),
        help_text=_("Create a optimizer with selected routers."))

    class Meta(object):
        name = _("Routers")
        permissions = ('openstack.services.network',)
        help_text = _("Select routers for your optimizer.")
...
class SelectRoutersStep(workflows.Step):
    ...
    template_name = "project/optimizers/_update_routers.html"
...
class AddPolicyAction(workflows.Action):
    ...
    class Meta(object):
        name = _("AddPolicy")
        permissions = ('openstack.services.network',)
        help_text = _("Create a optimizer policy with an ordered list "
                      "of optimizer rules.\n\n"
                      "A name must be given. Optimizer rules are "
...
class AddPolicy(workflows.Workflow):
    ...
    def handle(self, request, context):
        try:
            api.oaas.policy_create(request, **context)
            return True
        except Exception as e:
            msg = self.format_status_message(self.failure_message) + str(e)
            exceptions.handle(request, msg)
...
class AddOptimizerAction(workflows.Action):
    name = forms.CharField(max_length=80,
                           label=_("Name"),
                           required=False)
    description = forms.CharField(max_length=80,
    ...
    def __init__(self, request, *args, **kwargs):
        super(AddOptimizerAction, self).__init__(request, *args, **kwargs)

        optimizer_policy_id_choices = [('', _("Select a Policy"))]
        try:
            tenant_id = self.request.user.tenant_id
...
class AddOptimizerStep(workflows.Step):
    action_class = AddOptimizerAction
    contributes = ("name", "optimizer_policy_id", "description",
                   "admin_state_up")

    def contribute(self, data, context):
        context = super(AddOptimizerStep, self).contribute(data, context)
        context['admin_state_up'] = (context['admin_state_up'] == 'True')
        return context


class AddOptimizer(workflows.Workflow):
    slug = "addoptimizer"
#OaaS
    ...
    # oaas is designed to support a wide range of vendor
    # optimizers. Considering the multitude of vendor optimizer
    # features in place today, optimizer definition can
    # involve more complex configuration over time. Hence,
    # a workflow instead of a single form is used for
    # optimizer_rule add to be ready for future extension.
    default_steps = (AddOptimizerStep, )
    ...
    def handle(self, request, context):
        try:
            api.oaas.optimizer_create(request, **context)
            return True
        except Exception as e:
            msg = self.format_status_message(self.failure_message) + str(e)
            exceptions.handle(request, msg)

...
class OptimizerRouterInUse(nexception.InUse):
...
class Optimizerrouterinsertion(extensions.ExtensionDescriptor):
    """Extension class supporting Optimizer and Router(s) association.

    The extension enables providing an option to specify router-ids of
    routers where the optimizer is to be installed. This is supported in
    a manner so that the older version of the API continues to be supported.
    On a CREATE, if the router_ids option is not specified then the optimizer
    is installed on all routers on the tenant. If the router-ids option is
    provided with a list of routers then the optimizer is installed on the
    specified routers. If the router-ids option is provided with an empty
    list then the optimizer is created but put in an INACTIVE state to reflect
    that no routers are associated. This optimizer can be updated with a list
    ...
    set of routers that the optimizer is associated with.
    """
    @classmethod
    def get_name(cls):
        return "Optimizer Router insertion"

    @classmethod
    def get_alias(cls):
        return "oaasrouterinsertion"

    @classmethod
    def get_description(cls):
        return "Optimizer Router insertion on specified set of routers"

    @classmethod
    def get_namespace(cls):
        return ("http://docs.openstack.org/ext/neutron/oaasrouterinsertion"
            "/api/v1.0")

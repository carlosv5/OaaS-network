...
def get_optimizer_object_prefix(ri, opt):
    return get_router_object_prefix(ri) + '-' + opt['id'][:OBJ_PREFIX_LEN]
...
def get_optimizer_policy_name(ri, opt, rule):
    return get_optimizer_object_prefix(ri, opt) + rule['id'][:OBJ_PREFIX_LEN]

...
ZONE_OPTIMIZER_CMD = 'zone-policy/zone/{0}/from/{1}/optimizer/name/{2}'

LOG = logging.getLogger(__name__)


def get_optimizer_name(ri, opt):
    """Make optimizer name for Vyatta vRouter

    Vyatta vRouter REST API allows optimizer name length
    up to 28 characters.
    """
    return opt['id'].replace('-', '')[:28]

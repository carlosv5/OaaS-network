...
class CsrAclDriver(oaas_base.OaasDriverBase):
    """Cisco CSR ACL driver for OaaS.
    ...
    def __init__(self):
        LOG.debug("Initializing oaas CSR ACL driver")

    def _get_csr_host(self, optimizer_vendor_ext):
        settings = {
            'rest_mgmt_ip': optimizer_vendor_ext['host_mngt_ip'],
            'username': optimizer_vendor_ext['host_usr_nm'],
            'password': optimizer_vendor_ext['host_usr_pw'],
            'timeout': 30,
        }
        return cisco_csr_rest_client.CsrRestClient(settings)

    def _validate_optimizer_rule_data(self, optimizer):
        if 'optimizer_rule_list' not in optimizer:
            LOG.error(_LE("no rule list"))
            return False
        for rule in optimizer['optimizer_rule_list']:
            if 'name' not in rule:
    ...
    def _validate_optimizer_data(self, optimizer):
        data = ('admin_state_up', 'vendor_ext')
        is_valid = all(x in optimizer for x in data)
        if not is_valid:
            LOG.error(_LE("missing data in optimizer"))
            return is_valid

        data = ('host_mngt_ip', 'host_usr_nm', 'host_usr_pw', 'if_list')
        is_valid = all(x in optimizer['vendor_ext'] for x in data)
        if not is_valid:
            LOG.error(_LE("missing data in optimizer vendor_ext"))
            return is_valid

        for optimizer_interface in optimizer['vendor_ext']['if_list']:
            if optimizer_interface.get('direction', '') not in (
                'inside', 'outside', 'both'):
                LOG.error(_LE("invalid direction"))
                return False
            if 'port' not in optimizer_interface:
                LOG.error(_LE("no port"))
                return False
            port = optimizer_interface['port']
            if 'id' not in port:
    ...
    def _get_acl_rule_data(self, optimizer):
        """Get ACL RESTAPI request data from optimizer dictionary.
    ...
    def _post_acl_to_interfaces(self, optimizer, csr, acl_id, status_data):
    ...
    def _delete_acl_on_interface(self, csr, acl_id,
                                 csr_optimizer_interface_list):
        for interface in csr_optimizer_interface_list:
    ...
    def _create_optimizer(self, optimizer):
        ...
        LOG.debug("optimizer %s", optimizer)
        if not self._validate_optimizer_data(optimizer):
            return False, {}
        if not self._validate_optimizer_rule_data(optimizer):
            return False, {}

        csr = self._get_csr_host(optimizer['vendor_ext'])
        acl_data = self._get_acl_rule_data(optimizer)
        ...
        if not optimizer['admin_state_up']:
            LOG.debug("status %s", status_data)
            return True, status_data

        # apply ACL to interfaces
        self._post_acl_to_interfaces(optimizer, csr, acl_id, status_data)

        LOG.debug("status %s", status_data)
        return True, status_data

    def _delete_optimizer(self, optimizer):
        """Delete ACL.
        ...
        acl_id = optimizer['vendor_ext'].get('acl_id')
        if not acl_id:
            LOG.error(_LE("firewal (%s) has no acl_id"), optimizer['id'])
            return False
    ...
    def _update_optimizer(self, optimizer):
        ...
        :param optimizer: optimizer dictionry
        :return: True and status_data if OK
        ...
        csr = self._get_csr_host(optimizer['vendor_ext'])
        rest_acl_rules = self._get_acl_rule_data(optimizer)
        ...
        csr_opt_interface_list = self._get_acl_interface(csr, acl_id)
        self._delete_acl_on_interface(csr, acl_id, csr_opt_interface_list)

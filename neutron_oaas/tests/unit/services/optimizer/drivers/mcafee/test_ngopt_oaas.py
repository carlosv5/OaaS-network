...
import mock
import neutron_oaas.services.optimizer.drivers.mcafee as mcafee
import neutron_oaas.services.optimizer.drivers.mcafee.ngopt_oaas as oaas

from neutron.tests import base

FAKE_OPTIMIZER_ID = 'optimizer_id'
FAKE_POLICY_ID = 'policy_id'
FAKE_TENANT_ID = 'tenant_id'
FAKE_ROUTER_ID = 'router_id'
FAKE_FW_NAME = 'opt_name'


class NGFWOaasTestCase(base.BaseTestCase):

    def setUp(self):
        super(NGFWOaasTestCase, self).setUp()
        self.optimizer = oaas.NgoptOaasDriver()
        ...
        # we generate the policy name by formatting the ids of optimizer,
        # policy, router
        self.policy_name = "%s_%s_%s" % (
                FAKE_OPTIMIZER_ID[0:7], FAKE_POLICY_ID[0:7],
                FAKE_ROUTER_ID[0:7])
    ...
    def _fake_ipv4_rules(self):
        rule1 = {'action': 'deny',
                 ...
                 'destination_port': '23',
                 'enabled': True,
                 'optimizer_policy_id': FAKE_POLICY_ID,
                 'id': '1',
                 'ip_version': 4,
                 'name': 'a2',
                 'position': 1,
    ...
    def test_update_optimizer(self):
        ...
        optimizer = self._fake_optimizer(self.rule_list)

        ref_v4rule = self.tmp_ref + "/opt_ipv4_access_rule"
        ...
        with mock.patch('oslo_serialization.jsonutils.dumps',
            ...
            self.optimizer.update_optimizer('legacy', self.apply_list, optimizer)
            self.assertEqual(expected, post.call_args_list)

    def test_create_optimizer(self):
        self.test_update_optimizer()
    ...
    def test_delete_optimizer(self):
        ...
        with mock.patch.object(mcafee.smc_api.SMCAPIConnection, 'login'), \
            ...
            self.optimizer.delete_optimizer('legacy', self.apply_list, optimizer)

...
from neutron_oaas.services.optimizer.agents.varmour import varmour_router
from neutron_oaas.services.optimizer.drivers.varmour import varmour_oaas
...
class TestBasicRouterOperations(base.BaseTestCase):
    ...
    def _create_oaas(self):
        oaas = varmour_oaas.vArmourOaasDriver()
        oaas.rest.server = FAKE_DIRECTOR
        oaas.rest.user = 'varmour'
        oaas.rest.passwd = 'varmour'
        return oaas
    ...
    def _add_optimizer_rules(self, opt, rule_count=1):
        rules = []
        for i in range(rule_count):
            ...
            rules.append(rule)
        opt['optimizer_rule_list'] = rules

    def _prepare_optimizer_data(self):
        opt = {'id': _uuid(),
              'admin_state_up': True,
              'optimizer_rule_list': []}
        return opt

    def test_optimizer_without_rule(self):
        oaas = self._create_oaas()
        oaas.create_optimizer = mock.Mock()
        oaas.delete_optimizer = mock.Mock()
        ...
        rl = [ri]
        opt = self._prepare_optimizer_data()
        oaas.create_optimizer(self.fake_agent_mode, rl, opt)
        oaas.create_optimizer.assert_called_once_with(self.fake_agent_mode,
                                                      rl, opt)
        oaas.delete_optimizer(self.fake_agent_mode, rl, opt)
    ...
    def test_optimizer_with_rules(self):
        ...
        oaas.delete_optimizer = mock.Mock()
        opt = self._prepare_optimizer_data()
        self._add_optimizer_rules(opt, 2)
        ri = self._prepare_router_data(enable_snat=True)
        self._add_internal_ports(ri.router, port_count=1)
        self._add_floating_ips(ri.router, port_count=1)
        rl = [ri]
    ...
    def test_optimizer_add_remove_rules(self):
        oaas = self._create_oaas()
        oaas.create_optimizer = mock.Mock()
        oaas.delete_optimizer = mock.Mock()
        opt = self._prepare_optimizer_data()
        ...
        self._add_optimizer_rules(opt, 1)
        oaas.create_optimizer(self.fake_agent_mode, rl, opt)
        oaas.delete_optimizer(self.fake_agent_mode, rl, opt)

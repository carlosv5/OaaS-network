...
import neutron_oaas.services.optimizer.drivers.linux.iptables_oaas as oaas
...
FAKE_SRC_PORT = 5000
FAKE_DST_PORT = 22
FAKE_FW_ID = 'fake-opt-uuid'


class IptablesOaasTestCase(base.BaseTestCase):
    def setUp(self):
        super(IptablesOaasTestCase, self).setUp()
        ...
        self.optimizer = oaas.IptablesOaasDriver()

    def _fake_rules_v4(self, optid, apply_list):
        rule_list = []
        rule1 = {'enabled': True,
                 'action': 'allow',
                 'ip_version': 4,
        ...
        rule3 = {'enabled': True,
                 ...
                 'protocol': 'tcp',
                 'destination_port': '23'}
        ingress_chain = ('iv4%s' % optid)[:11]
        egress_chain = ('ov4%s' % optid)[:11]
    ...
    def _fake_optimizer_no_rule(self):
        rule_list = []
        opt_inst = {'id': FAKE_FW_ID,
                   'admin_state_up': True,
                   'tenant_id': 'tenant-uuid',
                   'optimizer_rule_list': rule_list}
        return opt_inst

    def _fake_optimizer(self, rule_list):
        opt_inst = {'id': FAKE_FW_ID,
                   'admin_state_up': True,
                   'tenant_id': 'tenant-uuid',
                   'optimizer_rule_list': rule_list}
    ...
    def _fake_optimizer_with_admin_down(self, rule_list):
        opt_inst = {'id': FAKE_FW_ID,
    ...
    def _setup_optimizer_with_rules(self, func, router_count=1,
        ...
        optimizer = self._fake_optimizer(rule_list)
        if distributed:
        ...
        func(distributed_mode, apply_list, optimizer)
        ...
        ingress_chain = 'iv4%s' % optimizer['id']
        egress_chain = 'ov4%s' % optimizer['id']
        bname = oaas.iptables_manager.binary_name
        ipt_mgr_ichain = '%s-%s' % (bname, ingress_chain[:11])
        ipt_mgr_echain = '%s-%s' % (bname, egress_chain[:11])
        for router_info_inst in apply_list:
            v4filter_inst = router_info_inst.iptables_manager.ipv4['filter']
            calls = [mock.call.remove_chain('iv4fake-opt-uuid'),
                     mock.call.remove_chain('ov4fake-opt-uuid'),
                     mock.call.remove_chain('oaas-default-policy'),
                     mock.call.add_chain('oaas-default-policy'),
                     mock.call.add_rule('oaas-default-policy', '-j DROP'),
                     ...
                     mock.call.add_rule('FORWARD',
                                        ...
                                        ipt_mgr_echain)),
                     mock.call.add_rule('FORWARD',
                                        '-o %s -j %s-oaas-defau' % (if_prefix,
                                        bname)),
                     mock.call.add_rule('FORWARD',
                                        '-i %s -j %s-oaas-defau' % (if_prefix,
                                        bname))]
            v4filter_inst.assert_has_calls(calls)

    def test_create_optimizer_no_rules(self):
        apply_list = self._fake_apply_list()
        optimizer = self._fake_optimizer_no_rule()
        self.optimizer.create_optimizer('legacy', apply_list, optimizer)
        invalid_rule = '-m state --state INVALID -j DROP'
        est_rule = '-m state --state ESTABLISHED,RELATED -j ACCEPT'
        bname = oaas.iptables_manager.binary_name

        for ip_version in (4, 6):
            ingress_chain = ('iv%s%s' % (ip_version, optimizer['id']))
            egress_chain = ('ov%s%s' % (ip_version, optimizer['id']))
            calls = [mock.call.remove_chain(
                     'iv%sfake-opt-uuid' % ip_version),
                     mock.call.remove_chain(
                         'ov%sfake-opt-uuid' % ip_version),
                     mock.call.remove_chain('oaas-default-policy'),
                     mock.call.add_chain('oaas-default-policy'),
                     mock.call.add_rule('oaas-default-policy', '-j DROP'),
                     mock.call.add_chain(ingress_chain),
                     ...
                     mock.call.add_rule(egress_chain, est_rule),
                     mock.call.add_rule('FORWARD',
                                        '-o qr-+ -j %s-oaas-defau' % bname),
                     mock.call.add_rule('FORWARD',
                                        '-i qr-+ -j %s-oaas-defau' % bname)]
            if ip_version == 4:
    ...
    def test_create_optimizer_with_rules(self):
        self._setup_optimizer_with_rules(self.optimizer.create_optimizer)

    def test_create_optimizer_with_rules_without_distributed_attr(self):
        self._setup_optimizer_with_rules(self.optimizer.create_optimizer,
                                        distributed=None)

    def test_create_optimizer_with_rules_two_routers(self):
        self._setup_optimizer_with_rules(self.optimizer.create_optimizer,
                                        router_count=2)

    def test_update_optimizer_with_rules(self):
        self._setup_optimizer_with_rules(self.optimizer.update_optimizer)

    def test_update_optimizer_with_rules_without_distributed_attr(self):
        self._setup_optimizer_with_rules(self.optimizer.update_optimizer,
                                        distributed=None)

    def _test_delete_optimizer(self, distributed=False):
        apply_list = self._fake_apply_list(distributed=distributed)
        optimizer = self._fake_optimizer_no_rule()
        self.optimizer.delete_optimizer('legacy', apply_list, optimizer)
        ingress_chain = 'iv4%s' % optimizer['id']
        egress_chain = 'ov4%s' % optimizer['id']
    ...
    def test_delete_optimizer(self):
        self._test_delete_optimizer()

    def test_delete_optimizer_without_distributed_attr(self):
        self._test_delete_optimizer(distributed=None)

    def test_create_optimizer_with_admin_down(self):
        apply_list = self._fake_apply_list()
        rule_list = self._fake_rules_v4(FAKE_FW_ID, apply_list)
        optimizer = self._fake_optimizer_with_admin_down(rule_list)
        self.optimizer.create_optimizer('legacy', apply_list, optimizer)
        calls = [mock.call.remove_chain('iv4fake-opt-uuid'),
                 mock.call.remove_chain('ov4fake-opt-uuid'),
                 mock.call.remove_chain('oaas-default-policy'),
    ...
    def test_create_optimizer_with_rules_dvr_snat(self):
        self._setup_optimizer_with_rules(self.optimizer.create_optimizer,
            distributed=True, distributed_mode='dvr_snat')

    def test_update_optimizer_with_rules_dvr_snat(self):
        self._setup_optimizer_with_rules(self.optimizer.update_optimizer,
            distributed=True, distributed_mode='dvr_snat')

    def test_create_optimizer_with_rules_dvr(self):
        self._setup_optimizer_with_rules(self.optimizer.create_optimizer,
            distributed=True, distributed_mode='dvr')

    def test_update_optimizer_with_rules_dvr(self):
        self._setup_optimizer_with_rules(self.optimizer.update_optimizer,
            distributed=True, distributed_mode='dvr')

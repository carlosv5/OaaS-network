...
class RouterInfo(object):
    ...
    def __str__(self):
        ...
        filename = "/tmp/carlos_router.txt"
        try:
            f = open(filename, "a")
        except IOError:
            print ".... ERROR in /tmp/carlos_router.txt"
            raise


        f.write("=========================================================\n")
        f.write("self.router_id: %s\n\n" % self.router_id)
        f.write("self.ex_gw_port: %s\n\n" % self.ex_gw_port)
        f.write("self._snat_enabled: %s\n\n" % self._snat_enabled)
        f.write("self.internal_ports: %s\n\n" % self.internal_ports)
        f.write("self.floating_ips: %s\n\n" % self.floating_ips)
        f.write("self.router: %s\n\n" % self.router)
        f.write("self.use_ipv6: %s\n\n" % self.use_ipv6)
        f.write("self.ns_name: %s\n\n" % self.ns_name)
        f.write("self.router_namespace: %s\n\n" % self.router_namespace)
        #f.write("self.iptables_manager: %s\n\n" % self.iptables_manager)
        f.write("self.routes: %s\n\n" % self.routes)
        f.write("self.agent_conf: %s\n\n" % self.agent_conf)
        f.write("self.driver: %s\n\n" % self.driver)
        f.write("self.radvd: %s\n\n" % self.radvd)

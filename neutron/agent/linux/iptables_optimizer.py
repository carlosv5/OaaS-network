...
from neutron.agent import optimizer
...
CHAIN_NAME_PREFIX = {optimizer.INGRESS_DIRECTION: 'i',
                     optimizer.EGRESS_DIRECTION: 'o',
                     SPOOF_FILTER: 's'}
DIRECTION_IP_PREFIX = {optimizer.INGRESS_DIRECTION: 'source_ip_prefix',
                       optimizer.EGRESS_DIRECTION: 'dest_ip_prefix'}
IPSET_DIRECTION = {optimizer.INGRESS_DIRECTION: 'src',
                   optimizer.EGRESS_DIRECTION: 'dst'}
...
class IptablesOptimizerDriver(optimizer.OptimizerDriver):
    """Driver which enforces security groups through iptables rules."""
    IPTABLES_DIRECTION = {optimizer.INGRESS_DIRECTION: 'physdev-out',
                          optimizer.EGRESS_DIRECTION: 'physdev-in'}
    ...
    def _setup_chains_apply(self, ports, unfiltered_ports):
        ...
        for port in ports.values():
            self._setup_chain(port, optimizer.INGRESS_DIRECTION)
            self._setup_chain(port, optimizer.EGRESS_DIRECTION)
            self.iptables.ipv4['filter'].add_rule(SG_CHAIN, '-j ACCEPT')
            self.iptables.ipv6['filter'].add_rule(SG_CHAIN, '-j ACCEPT')

        for port in unfiltered_ports.values():
            self._add_accept_rule_port_sec(port, optimizer.INGRESS_DIRECTION)
            self._add_accept_rule_port_sec(port, optimizer.EGRESS_DIRECTION)
    ...
    def _remove_chains_apply(self, ports, unfiltered_ports):
        for port in ports.values():
            self._remove_chain(port, optimizer.INGRESS_DIRECTION)
            self._remove_chain(port, optimizer.EGRESS_DIRECTION)
            self._remove_chain(port, SPOOF_FILTER)
        for port in unfiltered_ports.values():
            self._remove_rule_port_sec(port, optimizer.INGRESS_DIRECTION)
            self._remove_rule_port_sec(port, optimizer.EGRESS_DIRECTION)
    ...
    def _update_port_sec_rules(self, port, direction, add=False):
        ...
        if direction == optimizer.EGRESS_DIRECTION:
            jump_rule = ['-m physdev --%s %s --physdev-is-bridged '
                         '-j ACCEPT' % (self.IPTABLES_DIRECTION[direction],
                                        device)]
            if add:
    ...
    def _add_rules_by_security_group(self, port, direction):
        ...
        if direction == optimizer.EGRESS_DIRECTION:
            self._add_fixed_egress_rules(port,
                                         ipv4_iptables_rules,
                                         ipv6_iptables_rules)
        elif direction == optimizer.INGRESS_DIRECTION:
...
class OVSHybridIptablesOptimizerDriver(IptablesOptimizerDriver):
    ...
    def _get_jump_rule(self, port, direction):
        if direction == optimizer.INGRESS_DIRECTION:
    ...
    def _add_chain(self, port, direction):
        super(OVSHybridIptablesOptimizerDriver, self)._add_chain(port,
                                                                direction)
        if direction in [optimizer.INGRESS_DIRECTION,
                         optimizer.EGRESS_DIRECTION]:
            self._add_raw_chain_rules(port, direction)

    def _remove_chain(self, port, direction):
        super(OVSHybridIptablesOptimizerDriver, self)._remove_chain(port,
                                                                   direction)
        if direction in [optimizer.INGRESS_DIRECTION,
                         optimizer.EGRESS_DIRECTION]:
